...
do
    case $opt in
        "separated files")
            ...
            ;;
        "all in one")
            docker run -ti --rm -v $(BASE_DIR):/pdf -w /pdf pdf2htmlex/pdf2htmlex:0.18.8.rc2-master-20200820-alpine-3.12.0-x86_64 --zoom 1.3 --dest-dir output $(BASE_DIR)/src/yh_resume.pdf
            break
            ;;
        *) echo "無效的選項 $REPLY";;
    esac
